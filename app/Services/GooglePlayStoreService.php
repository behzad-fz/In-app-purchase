...
class GooglePlayStoreService implements ApplicationStoreInterface
{
    private $request;
    ...
    {
        $response =  Http::get('mock-api/https://sandbox.google.com/verifyReceipt',[
        ...
        if ($response->json()['response']) {
            $this->subscriptionModel->create([
               ...
               'receipt'    => $this->request->receipt,
               'purchase_date'  => Carbon::create($response->json()['info']['original_purchase_date'])->format('Y-m-d H:i:s'),
                'expiry_date'   => Carbon::create($response->json()['info']['expiration_date'])->format('Y-m-d H:i:s'),
    ...
    public function check($receipt)
    {
        Http::fake([
            ...
            'mock-api/https://sandbox.google.com/check*' => Http::response([
        ...
        if (is_numeric($receipt) && $receipt % 6 != 0) {
            $response = Http::retry(3, 100)->get('mock-api/https://sandbox.google.com/check');
        } else {
            $response =  Http::get('mock-api/https://sandbox.google.com/check') ;
        }
